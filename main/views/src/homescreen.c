...
void homescreen_init(void) {
    ...
    lv_style_set_bg_color(&_screen_style, lv_color_hex(0x000000));
    lv_style_set_border_width(&_screen_style, 0);    
    lv_style_set_border_color(&_screen_style, lv_color_hex(0x000000));
    lv_style_set_radius(&_screen_style, 0);


    // Create blank screen with black background style
    screen = lv_obj_create(lv_scr_act());
    lv_obj_set_size(screen, 240, 320);
    lv_obj_add_style(screen, &_screen_style, 0);
    lv_scr_load(screen);
